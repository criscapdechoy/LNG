...
class LatticeStructure:
    def __init__(self, nodes, edges = [], faces = [], sym =[], dim=[], N = [], boundary = [], nodeR = 0.0, cros_sec_bound=[], atr1 =[], atr2 =[], atr3 =[], atr4 =[], atr5 =[]):
        self.nodes = nodes
        self.edges = edges
        self.faces = faces
        self.sym = sym
    ...
    def size(abc):
        print("\n mesh: ", abc.nodes.shape)
        print("\n edges: ", abc.edges.shape)
        print("\n faces: ", abc.faces.shape )
        print("\n boundary: ", abc.boundary.shape)
...
#    print('get_faces(N, t, sizeXYZ, coord, edges, faces=[], centres=[], show = False F) // newcoord, faces, edges, N, bound_node G')
    ...
    if LS.nodeR!=0:
        try:
            newface =[]
            for f in LS.faces:
                LS.nodes, LS.edges, newf, LS.N = gen_thicknes_data(LS.N, LS.nodes[f], LS.t, sizeXYZ, LS.edges)
                newface.extend(newf)
        except:
            LS.nodes, LS.edges, newface, LS.N = gen_thicknes_data(LS.N, LS.nodes[f], LS.t, sizeXYZ, LS.edges)
#
...
#            plt.ylim(-0,5)
#            plt.show()
        LS.bound_node = set(range(Nn, LS.N ))
...
#    print('gen_mesh( N, nodes, edges, faces, bound_node, sizeXYZ, meshSize=[], DoMesh = False F // nodes, edges, new_faces, N, bound_node G')
    if not(DoMesh):
        ...
        newc = []
        k =-1
        LS.edges = [[a] for a in range(LS.N)]
        for edge in old_edges:
            for no in edge[1:]:
                ...
                for i in range(1,div):
                    ...
                    nedges.append(k+LS.N)
                    LS.edges.append([k+LS.N])
                if len(LS.aw) !=0:
...
#        LS.N = len(LS.nodes) # new number of nodes
        new_faces = LS.faces
...
#    print('gen_nodegrid(coord, N, sym, n_G, DIM, sizeXYZ, shape,f=[1,1,1] F) // mesh, ind_hash, num, delmax, Boundary G')
    ...
    num = np.matrix(np.zeros([n_G[0]*n_G[1]*n_G[2],LS.N]))
    ...
    for i in range(msize[0]):
        for j in range(msize[1]):
            for k in range(msize[2]):
                ...
                coords[:,indbol] = np.reshape(np.repeat([delmax[indbol]], LS.N,axis=0),(LS.N,sum(indbol))) - LS.nodes[:,indbol]
                coordS[i,j,k] = coords
                iis = np.array(ii)
...
#            
            ...
            for i in range(n_G[0]):
                if 0 in LS.sym:
                    ...
                    iT = 0
                p = p + 1
                num_r = np.reshape(ind_elem_GS[iT,jT,kT] + i*(n_L[0]-1) + k*nlayer+ j*nplane,(1,LS.N))
                num[p,:] =  num_r
                index[num_r] = np.ones([len(num_r),1])
                [dx,dy,dz] = [float(delmax[0]*i), float(delmax[1]*j), float(delmax[2]*k)]
                mesh[num_r,:] = coordS[iT,jT,kT] + np.repeat([[dx,dy,dz]], LS.N, axis=0) + do_shapeME(
