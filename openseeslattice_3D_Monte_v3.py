...
os.chdir(folder)
folder = './TEST' + shape +'t'+str(base_element)+'s'+ ''.join(map(str, n_G[:])) +'p'+str(population) +'Monte_v2' +str(sample)
LNG_engine.createFolder(folder)
os.chdir(folder)
...
LS.nodes, edges, _, B, Nedges = base.main(base_element, shape, n_G, LS.nodesSize=[h/2, h*(2**0.5)/2], sizeXYZ=(L,L,L), t=0.0, ExtractGeom=False, View=False)
...
for p in range(population):
    ...
    del(elapsed, w0, w, v0, v, u0, u, ii, e_i)
