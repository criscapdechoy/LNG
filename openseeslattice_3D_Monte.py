...
sample=np.random.normal(loc=0.10, scale=0.10/4, size=5000)
...
folder = './TEST' + shape +'t'+str(base_element)+'s'+ ''.join(map(str, n_G[:])) + 'Monte' +str()
...
for p in sample:
    ...
    mesh, e_i, faces_i, B = base.main(base_element, shape, n_G, meshSize=[h/2, h*(2**0.5)/2], sizeXYZ=(L,L,L), t=0.0, ExtractGeom=False, View=False, percent=p)
...
hist0, bins0 = np.histogram(sample[W[:,2]!=0.0], bins=int(3.322*math.log(len(sample))-2), weights = np.ones(np.shape(normalW))/len(normalW))  # arguments are passed to np.histogram
