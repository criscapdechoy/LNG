...
class LatticeStructure:
    ...
    def __init__(self, nodes, edges = [], faces = [], sym =[], dim=[], boundary = [], N =[], nodeR = 0.0, cros_sec_bound=[], atr1 =[], atr2 =[], atr3 =[], atr4 =[], atr5 =[]):
...
def get_basic_elem( e ):
    ...
    if e == 0:
        ...
        sym = [0,1,2]
        dim = (0,1,2)
    LS = LatticeStructure(nodes, edges, faces, sym, dim)
...
def get_input(LS, sizeXYZ):
    if type(sizeXYZ)!= np.array:
        sizeXYZ = np.asmatrix(sizeXYZ)
    # Amount of nodes
    scale = np.divide(sizeXYZ[:,LS.dim],np.max(LS.nodes, axis=0))
    LS.nodes = np.multiply(LS.nodes,scale)
    # Adding missing dimension in case of node coordinates given in 2D
    if np.shape(LS.nodes)[1] != 3:
        LS.nodes = np.array([LS.nodes[:,0],np.zeros([np.shape(LS.nodes)[0],1]),LS.nodes[:,1]]).T[0]
        scale = np.array([scale[:,0],np.zeros([np.shape(scale)[0],1]),scale[:,1]]).T[0]
    T_edges = [[a] for a in range(len(LS.nodes))]
    for i,j in LS.edges:
...
def check_boundary(num_r, mesh_r, ii, i, j, k, n_L, n_G, delmax, Boundary, shape, dim):
#    print('check_boundary(num_r, mesh_r, ii, i, j, k, n_L, n_G, delmax, Boundary, shape, dim F) // Boundary G')
    if (i == 0 or n_G[0]) and (j ==0 or n_G[1]) and (k ==0 or n_G[2]):
        for i in dim:
            b0 = (num_r[abs(mesh_r[num_r,i] - do_shapeME(np.zeros([1,3]), n_L, n_G, delmax, shape, ii)[0,i])<= 10**-5])
            for b in b0:
                if b not in Boundary[(i*2)]:
                    Boundary[(i*2)].append(b)
            b0 = (num_r[abs(mesh_r[num_r,i] - np.multiply(n_G, delmax)[i]-do_shapeME(np.array([np.multiply(n_G, delmax)]), 
                        n_L, n_G, delmax, shape, ii)[0,i])<=10**-5])
            for b in b0:
    ...
    for row in np.array(num):
        ...
        for node_in in row:
            ...
            for node_out in node_con[1:]:
                ...
                if node_out not in edges_i[node_in]:
                    edges_i[node_in].append(node_out)
    return edges_i

def gen_CAD(filename, LS, lim, Export, View, foldername = ''):
...
def get_faces(LS, sizeXYZ,show = False):
#    print('get_faces(N, t, sizeXYZ, coord, edges, faces=[], centres=[], show = False F) // newcoord, faces, edges, N, bound_node G')
    ...
    if LS.nodeR!=0:
        try:
            newface =[]
            for f in LS.faces:
                LS.nodes, LS.edges, newf, LS.N = gen_thicknes_data(len(LS.nodes), LS.nodes[f], LS.nodeR, sizeXYZ, LS.edges)
                newface.extend(newf)
        except:
            LS.nodes, LS.edges, newface, LS.N = gen_thicknes_data(len(LS.nodes), LS.nodes[f], LS.nodeR, sizeXYZ, LS.edges)
#
...
#            plt.ylim(-0,5)
#            plt.show()
        LS.bound_node = set(range(Nn, len(LS.nodes) ))
#        newcoord = np.asarray(newcoord)
#        N = len(newcoord) 
    else:
        LS.bound_node = set()
        newface=[[]]
    LS.faces = np.asarray(newface)
    return LS
...
def gen_mesh( LS, sizeXYZ, meshSize=[], DoMesh = False):
#    print('gen_mesh( N, nodes, edges, faces, bound_node, sizeXYZ, meshSize=[], DoMesh = False F // nodes, edges, new_faces, N, bound_node G')
    if not(DoMesh):
        ...
        old_edges = copy.deepcopy(LS.edges)
        newc = []
        k =-1
        LS.edges = [[a] for a in range(len(LS.nodes))]
        for edge in old_edges:
            for no in edge[1:]:
                nedges = []
                ni=edge[0]
                vect = LS.nodes[no]-LS.nodes[ni]
                dist = np.linalg.norm(vect)
                div = 1
                while ((dist>maxsize) and ((dist/(div+1)>=minsize) and (dist/(div)>maxsize))):
                    div = div + 1
                newcoord = LS.nodes[ni] 
                # Add new edges and new nodes to the data
                for i in range(1,div):
                    k = k + 1
                    newcoord = newcoord + vect*(1./div)
                    newc.append(newcoord)
                    nedges.append(k+len(LS.nodes))
                    LS.edges.append([k+len(LS.nodes)])
                if len(LS.cros_sec_bound) !=0:
                    if (ni in LS.cros_sec_bound) and (no in LS.cros_sec_bound):
                        # Add new bound nodes to the bound node set
                        LS.cros_sec_bound = LS.cros_sec_bound.union(set(nedges))
                if len(nedges) == 0:
                    LS.edges[ni].append(no)
#                    try:
...
##                            pass
                if len(nedges) != 0:
                    LS.edges[ni].append(nedges[0])
                    LS.edges[no].append(nedges[-1])
                if len(nedges)>=2:
                    for nn in nedges[0:-1]:
                        LS.edges[nn].append(nn+1)  
        # Add new nodes to the node array
        LS.nodes = np.concatenate((LS.nodes, np.reshape(newc,(len(newc),3))))
#        len(LS.nodes) = len(LS.nodes) # new number of nodes
        new_faces = LS.faces

        # TRIANGULATE faces if needed
        if (np.sum(LS.faces) != 0) and (sizeXYZ!=0):
            triang = Delaunay(LS.nodes[:,[0,2]])
            triangg = do_trimming(triang.simplices, LS.cros_sec_bound)
#            delaunay_plot_2d(triang)
            plt.figure()
            plt.axis('equal')
            plt.axis('off')
            plt.triplot(LS.nodes[:,0], LS.nodes[:,2], triangg)
            plt.show()
...
def gen_nodegrid(LS, n_G, sizeXYZ, shape):
#    print('gen_nodegrid(coord, N, sym, n_G, DIM, sizeXYZ, shape,f=[1,1,1] F) // mesh, ind_hash, num, delmax, Boundary G')
    ...
    n_G = check_dimensions( LS.dim, n_G)
    n_L, ii, delmax, delta = do_pregrid(LS.nodes, sizeXYZ)
    ...
    num = np.matrix(np.zeros([n_G[0]*n_G[1]*n_G[2],len(LS.nodes)]))
    # Initialize Boundary: List of the list of boundary nodes of the basic element. 
    LS.boundary = [[],[],[],[],[],[]] # [min(x), max(x), min(y), max(y), min(z), max(z)]
    p = -1
    if LS.sym !=-1:
        msize = [1,1,1]
        for rep in LS.sym:
            msize[rep] = 2
        msize.append(len(LS.nodes))
    ...
    for i in range(msize[0]):
        for j in range(msize[1]):
            for k in range(msize[2]):
                ...
                coords = np.array(LS.nodes)
                coords[:,indbol] = np.reshape(np.repeat([delmax[indbol]], len(LS.nodes),axis=0),(len(LS.nodes),sum(indbol))) - LS.nodes[:,indbol]
                coordS[i,j,k] = coords
                iis = np.array(ii)
...
#            
            ...
            for i in range(n_G[0]):
                if 0 in LS.sym:
                    ...
                    iT = 0
                p = p + 1
                num_r = np.reshape(ind_elem_GS[iT,jT,kT] + i*(n_L[0]-1) + k*nlayer+ j*nplane,(1,len(LS.nodes)))
                num[p,:] =  num_r
                index[num_r] = np.ones([len(num_r),1])
                [dx,dy,dz] = [float(delmax[0]*i), float(delmax[1]*j), float(delmax[2]*k)]
                mesh[num_r,:] = coordS[iT,jT,kT] + np.repeat([[dx,dy,dz]], len(LS.nodes), axis=0) + do_shapeME(
                        coordS[iT,jT,kT]+[dx,dy,dz], n_L,n_G,delmax,shape,iiS[iT,jT,kT], i,j,k)
                LS.boundary = check_boundary(num_r, mesh,iiS[iT,jT,kT],i,j,k,n_L,n_G,delmax,LS.boundary,shape,LS.dim)
    ...
    LS.nodes = mesh[index]
    LS.N = len(LS.nodes)
    del(mesh)#Take only the nodes of interest from the rectangular grid
    ...
    for i in index:
        k = k + 1
        ind_hash[i] = k
    LS.boundary = [do_translate(list,  ind_hash) for list in LS.boundary]
    del(indbol)
    return LS, ind_hash, num, delmax
